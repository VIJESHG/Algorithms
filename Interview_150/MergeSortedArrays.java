...
public class MergeSortedArrays {
    public void merge(int[] nums1, int m, int[] nums2, int n) {
        ...
        int p1 = m - 1; // Pointer for the last element in nums1's valid range
        int p2 = n - 1; // Pointer for the last element in nums2
        int p = m + n - 1; // Pointer for the last position in nums1

        // Traverse from the end and place the largest element in the correct position
        while (p1 >= 0 && p2 >= 0) {
    ...
    public static void main(String[] args) {
        ...
        n = 0;
        solution.merge(nums1, m, nums2, n);
        System.out.println(java.util.Arrays.toString(nums1)); // [1]
